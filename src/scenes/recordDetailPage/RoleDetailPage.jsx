...
const RoleDetailPage = ({ item }) => {
  ...
  const formSubmission = (values) => {
    ...
    console.log(values.createdDate, "values.createdDate in form submission");
    console.log("single role", singleRole);
    let dateSeconds = new Date().getTime();
    let createDateSec =
      Date.parse(
        values.createdDate.replace(
          /(\d{2})\/(\d{2})\/(\d{4}), (\d{2}):(\d{2}):(\d{2})/,
          "$2/$1/$3 $4:$5:$6"
        )
      ) || new Date(values.createdDate).getTime();

    console.log(createDateSec, "createDateSec in form submission");
    ...
    if (showNew) {
      ...
      console.log("inside show old");
      console.log(createDateSec, "createDateSec");
      values.modifiedDate = dateSeconds;
